...
function rowsFromExec(execResult: any[]): any[] {
...
export function getProjectsIPC() {
  const res = db.exec('SELECT * FROM projects ORDER BY created_at DESC')
  return rowsFromExec(res)
}

/** Crea un proyecto y devuelve { id } */
export function createProjectIPC(_event: any, payload: { name: string; description?: string; budget?: number }) {
...
export function getMembersIPC() {
  const res = db.exec('SELECT * FROM members ORDER BY name')
  return rowsFromExec(res)
}

/** Crea un miembro y devuelve { id } */
export function createMemberIPC(_event: any, payload: { name: string; role?: string; email?: string }) {
...
export function addExpenseIPC(_event: any, payload: { projectId: number; memberId?: number; amount: number; description?: string }) {
...
export function getExpensesIPC(_event: any, projectId: number) {
 ...
 */
export async function generateBudgetPdf(projectId: number): Promise<string> {
  try {
